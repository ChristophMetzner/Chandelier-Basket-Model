...
def plot_MEG_PSD_combination(time,freqs,meg_data,meg_data_s,psd_data,psd_data_s,xlabel1,ylabel1,xlabel2,ylabel2,annotations,savefig,filename,fontsizes):
    f,((ax1,ax2),(ax3,ax4)) = plt.subplots(2,2,sharex=False,sharey=False,figsize=[30.0,20.0])
...
def plot_barplot2(x,ctrl,schiz,xlabel,ylabel,annotations,colours,savefig,filename,fontsizes,tick_label):
    fig = plt.figure(figsize=(30, 20))
    gs1 = gridspec.GridSpec(1, 3)
    gs1.update(top=0.95, bottom=0.55)
    ax1 = plt.subplot(gs1[:,0])
    ax2 = plt.subplot(gs1[:,1])
    ax3 = plt.subplot(gs1[:,2])
    gs2 = gridspec.GridSpec(1, 2)
    gs2.update(top=0.45, bottom=0.05, left=0.25, right= 0.75)
