...
class simpleModelChandelier(object):
    ...
    def __init__(self,n_ex=20,n_bask=10,n_chand=10,eta=5.0,tau_R=0.1,tau_ex=2.0,tau_bask=8.0,tau_chand=8.0,g_ee=0.015,g_eb=0.025,g_ec=0.025,g_be=0.015,g_ce=0.015,g_bb=0.02,g_cc=0.02,g_bc=0.02,g_de=0.3,g_db=0.08,g_dc=0.08,dt=0.05,b_ex=-0.01,b_bask=-0.01,b_chand=-0.01,drive_frequency=0.0,background_rate=33.3,A=0.5,seed=12345,filename='default',directory='/'):
        self.n_ex = n_ex
        self.n_bask = n_bask
    ...
    def run(self,time=100.0,saveMEG=0,saveEX=0,saveBASK=0,saveCHAND=0):
        '''
        ...
        for t in range(1,len(time_points)):
            ...
            s_be[:,:,t] 	= s_be[:,:,t-1] + self.dt*(-1.0*(s_be[:,:,t-1]/self.tau_bask) + np.exp(-1.0*self.eta*(1+np.cos(b)))*((1.0-s_be[:,:,t-1])/self.tau_R))
            s_bb[:,:,t] 	= s_bb[:,:,t-1] + self.dt*(-1.0*(s_bb[:,:,t-1]/self.tau_bask) + np.exp(-1.0*self.eta*(1+np.cos(theta_bask[:,t-1])))*((1.0-s_bb[:,:,t-1])/self.tau_R))
            s_bc[:,:,t] 	= s_bc[:,:,t-1] + self.dt*(-1.0*(s_bc[:,:,t-1]/self.tau_bask) + np.exp(-1.0*self.eta*(1+np.cos(b)))*((1.0-s_bc[:,:,t-1])/self.tau_R))
            for m in range(self.n_chand):
                c[m,0]=theta_chand[m,t-1]
            s_ce[:,:,t] 	= s_ce[:,:,t-1] + self.dt*(-1.0*(s_ce[:,:,t-1]/self.tau_chand) + np.exp(-1.0*self.eta*(1+np.cos(c)))*((1.0-s_ce[:,:,t-1])/self.tau_R))
            s_cc[:,:,t] 	= s_cc[:,:,t-1] + self.dt*(-1.0*(s_cc[:,:,t-1]/self.tau_chand) + np.exp(-1.0*self.eta*(1+np.cos(theta_chand[:,t-1])))*((1.0-s_cc[:,:,t-1])/self.tau_R))
            ...
            S_bask[:,t] = self.g_eb*np.sum(s_eb[:,:,t-1],axis=0) - self.g_bb*np.sum(s_bb[:,:,t-1],axis=0) + self.g_db*s_db[:,t-1]
            S_chand[:,t] = self.g_ec*np.sum(s_ec[:,:,t-1],axis=0) - self.g_cc*np.sum(s_cc[:,:,t-1],axis=0) - self.g_bc*np.sum(s_bc[:,:,t-1],axis=0) + self.g_dc*s_dc[:,t-1] 
            ...
            theta_bask[:,t] 	= theta_bask[:,t-1] + self.dt*( (1 - np.cos(theta_bask[:,t-1])) + (B_bask + S_bask[:,t] + N_bask[:,t])*(1 + np.cos(theta_bask[:,t-1])))
            theta_chand[:,t] 	= theta_chand[:,t-1] + self.dt*( (1 - np.cos(theta_chand[:,t-1])) + (B_chand + S_chand[:,t] + N_chand[:,t])*(1 + np.cos(theta_chand[:,t-1])))
